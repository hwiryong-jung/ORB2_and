        ...
        mbDeactivateLocalizationMode(false)
{
    ...
    if(mSensor==MONOCULAR) LOG("## MONOCULAR ^^");
    else if(mSensor==STEREO) LOG("## STEREO ^^");
    else if(mSensor==RGBD) LOG("## RGBD ^^");
    ...
    if(!fsSettings.isOpened())
    {
    	...
    	//__android_log_print(ANDROID_LOG_ERROR,"jni", "## file open error : %s", strSettingsFile);
       exit(-1);
    }
