 ...
 * Signature: ()V
 */
JNIEXPORT void JNICALL Java_orb_slam2_android_nativefunc_OrbNdkHelper_glesInit(JNIEnv *env, jclass cls)
{
...
(JNIEnv * env, jclass cls)
{
	...
	//if(tmp<1.0f) tmp += 0.002f;
	//else         tmp = 0.0f;
	//glClearColor(1.0f*tmp, 1.0f*tmp, .0f*tmp, 0.0f);
	glClear (GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glMatrixMode (GL_MODELVIEW);
	glLoadIdentity ();

	if(init_end) s->drawGL();
}
