...
public class ORBSLAMForDataSetActivity extends Activity implements OnClickListener,
	...
	Button start, stop;
	static String vocPath = "/storage/emulated/0/ORBvoc.txt", 
			calibrationPath = "/storage/emulated/0/TUM3.yaml", 
			ImgPath = "/storage/emulated/0/tum_dataset/rgbd_dataset_freiburg3_long_office_household/rgb/";
	LinearLayout linear;
	...
	protected void onCreate(Bundle savedInstanceState) 
	{
		...
		{
			Toast.makeText(ORBSLAMForDataSetActivity.this, "init has been started!", Toast.LENGTH_LONG).show();
        				...
        				public void run() 
        				{
        					if (!TextUtils.isEmpty(ImgPath)) 
        					{
        						...
        						if (dir.isDirectory()) 
        						{
        							...
        							{
        								tmp = BitmapFactory.decodeFile(file.getAbsolutePath());
        								...
        								timestamp = Double.parseDouble(file.getName().substring(0, file.getName().length() - 5));
        								
        								int w = tmp.getWidth(), h = tmp.getHeight();
        								int[] pix = new int[w * h];
        								tmp.getPixels(pix, 0, w, 0, 0, w, h);

        								int[] resultInt = OrbNdkHelper.startCurrentORB(timestamp, pix, w, h);//feelix
        								resultImg = Bitmap.createBitmap(w, h, Config.RGB_565);
        								resultImg.setPixels(resultInt, 0, w, 0, 0, w, h);//feelix
        								runOnUiThread(new Runnable() {
        									@Override
        									public void run() {        										
        								...
        								//Log.e(" ","## PNG2 ^^"+file.getAbsolutePath());
        							}
        						}
        					} 
        					else 
        					{
        						Toast.makeText(ORBSLAMForDataSetActivity.this, "empty images", Toast.LENGTH_LONG).show();
        					}
        				}
        			}).start();
                       break;   
